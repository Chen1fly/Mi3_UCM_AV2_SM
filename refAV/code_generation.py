...
def predict_scenario_from_description(natural_language_description, output_dir: Path,
    ...
    with open(paths.AV2_CATEGORIES, 'r', encoding='UTF-8', errors='ignore') as f:
        av2_categories = f.read().format()
    # with open(paths.PREDICTION_EXAMPLES, 'r') as f:
    #     prediction_examples = f.read().format()

    # prompt = f"Please use the following functions to find instances of a referred object in an autonomous driving dataset. Be precise to the description, try to avoid returning false positives. {refav_context} \n {av2_categories}\n Define a single scenario for the description:{natural_language_description}\n Here is a list of examples: {prediction_examples}. Only output code and comments as part of a Python block. Feel free to use a liberal amount of comments. Do not define any additional functions, or filepaths. Do not include imports. Assume the log_dir and output_dir variables are given. Wrap all code in one python block and do not provide alternatives. Output code even if the given functions are not expressive enough to find the scenario.If you use has objects in relative direction(), being crossed by(), heading in relative direction to(), function, direction parameter specifies the orientation of related candidates relative to track candidates. The facing toward() and heading toward() functions indicate that the track candidates parameter is oriented toward the related candidates parameter. "
    ...
    if err != '':
        prompt = f"Please use the following functions to find instances of a referred object in an autonomous driving dataset. Be precise to the description, try to avoid returning false positives. {refav_context} \n {av2_categories}\n Define a single scenario for the description:{natural_language_description}\n Here is a list of examples: {prediction_examples}. Only output code and comments as part of a Python block. Feel free to use a liberal amount of comments. Do not define any additional functions, or filepaths. Do not include imports. Assume the log_dir and output_dir variables are given. Wrap all code in one python block and do not provide alternatives. Output code even if the given functions are not expressive enough to find the scenario.If you use has objects in relative direction(), being crossed by(), heading in relative direction to(), function, direction parameter specifies the orientation of related candidates relative to track candidates. The facing toward() and heading toward() functions indicate that the track candidates parameter is oriented toward the related candidates parameter.This is the code generated last time: {code}, with the error message: {err}. Please avoid code runtime errors. "
