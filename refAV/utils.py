    ...
    def wrapper(track_candidates, log_dir, *args, **kwargs):
        ...
        for track_uuid, unfiltered_related_objects in track_dict.items():
            if true_tracks.get(track_uuid, None) is not None:
                prior_related_objects = scenario_at_timestamps(unfiltered_related_objects,
...
def composable_relational(composable_func):
    ...
    def wrapper(track_candidates, related_candidates, log_dir, *args, **kwargs):
        ...
        _, relationship_dict = parallelize_uuids(composable_func, track_uuids, candidate_uuids, log_dir, *args,
                                                 **kwargs)
        ...
        for track_uuid, unfiltered_related_objects in relationship_dict.items():
            for related_uuid, related_timestamps in unfiltered_related_objects.items():
                eligible_timestamps = sorted(
                    set(related_timestamps).intersection(get_scenario_timestamps(track_dict[track_uuid])))
                scenario_dict[track_uuid][related_uuid] = scenario_at_timestamps(related_candidate_dict[related_uuid],
                                                                                 eligible_timestamps)
...
def has_free_will(track_uuid, log_dir):
    ...
    if category in ['ANIMAL', 'OFFICIAL_SIGNALER', 'RAILED_VEHICLE', 'ARTICULATED_BUS', 'WHEELED_RIDER', 'SCHOOL_BUS',
                    'MOTORCYCLIST', 'TRUCK_CAB', 'VEHICULAR_TRAILER', 'BICYCLIST', 'MOTORCYCLE', 'TRUCK', 'BOX_TRUCK',
                    'BUS',
...
def get_road_side(ls: LaneSegment, log_dir, side: Literal['same', 'opposite'], avm=None) -> list[LaneSegment]:
    ...
    while same_side_frontier:
        ...
        if lane_segment.left_neighbor_id and lane_segment.left_neighbor_id in map_lane_ids:
            ...
            edge_distance = np.linalg.norm(left_edge[0] - right_edge[0]) + np.linalg.norm(
...
def get_nth_pos_deriv(
    ...
    for _ in range(n):
        ...
        for i in range(len(prev_deriv)):
            ...
            next_deriv[i] = 1e9 * (prev_deriv[future_index] - prev_deriv[past_index]) / (
    ...
    if n == 0 and coordinate_frame == get_ego_uuid(log_dir):
        for i in range(len(prev_deriv)):
            ...
            cf_angles[i] = Rotation.from_matrix(
                city_to_ego @ Rotation.from_rotvec(prev_deriv[i]).as_matrix()).as_rotvec()
        ...
        if not cf_df.empty:
            ...
            for i in range(len(prev_deriv)):
                ...
                cf_angles[i] = Rotation.from_matrix(
                    city_to_obj @ Rotation.from_rotvec(prev_deriv[i]).as_matrix()).as_rotvec()
    elif n == 0 and type(coordinate_frame) == SE3:
        for i in range(len(prev_deriv)):
            cf_angles[i] = Rotation.from_matrix(
                coordinate_frame.rotation @ Rotation.from_rotvec(prev_deriv[i]).as_matrix()).as_rotvec()
    ...
    for track_uuid, related_objects in scenario.items():
        if isinstance(related_objects, dict):
            ...
            track_av2_timestamps = np.array(
                sorted(log_df.loc[log_df['track_uuid'] == track_uuid, 'timestamp_ns'].unique()))
            ...
            if timestamps and related_uuid not in track_dict and related_uuid not in all_related_objects:
                ...
                if non_track_timestamps:
                    all_related_objects[related_uuid] = sorted(
                        set(all_related_objects[related_uuid]).union(non_track_timestamps))
...
def create_mining_pkl(description, scenario, log_dir: Path, output_dir: Path, method_name='ref'):
    ...
    save(sequences, str(output_dir / log_id / f'{description}_predictions.pkl'))
