...
def get_objects_in_relative_direction(
    ...
    tracked_objects = \
        reverse_relationship(has_objects_in_relative_direction)(track_candidates, related_candidates, log_dir,
                                                                direction,
...
def changing_lanes(
    ...
    for index in lane_changes_exact['left']:
        ...
        while lane_change_start > 0:
            ...
            if (
                    pos_along_width0 and pos_along_width1 and pos_along_width0 > pos_along_width1) or lane_change_start == index - 1:
        ...
        while lane_change_end < len(timestamps):
            ...
            if (
                    pos_along_width0 and pos_along_width1 and pos_along_width0 > pos_along_width1) or lane_change_end == index:
...
def on_lane_type(
    ...
    return [timestamp for timestamp in timestamps if
            scenario_lanes[timestamp] and scenario_lanes[timestamp].lane_type == lane_type]
        ...
        for i in range(len(timestamps)):
            if track_lanes[timestamps[i]] and candidate_lanes[timestamps[i]] \
                    and (((track_lanes[timestamps[i]].id == candidate_lanes[timestamps[i]].id \
                          ...
                          and track_length < candidate_pos[i, 0] < FOLLOWING_THRESH and -LATERAL_TRHESH < candidate_pos[
                              i, 1] < LATERAL_TRHESH \
                         ...
                         and (-track_width <= candidate_pos[i, 1] + candidate_width <= track_width or -track_width <=
                              candidate_pos[i, 1] - candidate_width <= track_width) \
                         and candidate_heading_similarity[i] > HEADING_SIMILARITY_THRESH):
...
def heading_in_relative_direction_to(track_candidates, related_candidates, log_dir,
    ...
    for related_uuid in related_candidates:
        ...
        for i, timestamp in enumerate(track_timestamps):
            ...
            if timestamp in related_timestamps:
                ...
                if np.linalg.norm(track_dir) < 1 and has_free_will(track_uuid, log_dir) and np.linalg.norm(
                        related_dir) > 1:
                    track_cuboid = get_cuboid_from_uuid(track_uuid, log_dir, timestamp=timestamp)
                    track_self_dir = np.array([1, 0, 0])

                    timestamp_track_pos = track_pos[i]
                    timestamp_track_posx = ego_to_city[timestamp].compose(track_cuboid.dst_SE3_object).transform_from(
                        track_self_dir)
                    track_dir = timestamp_track_posx - timestamp_track_pos

                elif np.linalg.norm(related_dir) < 1 and has_free_will(related_uuid, log_dir) and np.linalg.norm(
                        track_dir) > .5:
                    ...
                    timestamp_related_posx = ego_to_city[timestamp].compose(
                        related_cuboid.dst_SE3_object).transform_from(related_x_dir)
...
def on_relative_side_of_road(
    ...
    avm = get_map(log_dir)
    track_ls = get_scenario_lanes(track_uuid, log_dir, avm=avm)
    semantic_lanes = {timestamp: get_road_side(track_ls[timestamp], log_dir, side=side, avm=avm) for timestamp in
                      timestamps}
...
def output_scenario(
    ...
    if visualize:
        ...
        visualize_scenario(scenario, log_dir, log_scenario_visualization_path, description=description,
                           **visualization_kwargs)
