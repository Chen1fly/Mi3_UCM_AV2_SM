...
def plot_cuboids(cuboids: list[Cuboid], plotter: pv.Plotter, transforms: list[SE3], color='red', opacity=.25,
                 with_front=False,
    ...
    if with_cf:
        all_cfs_actor = plotter.add_mesh(combined_cfs, color='black', line_width=3, opacity=opacity, pickable=False,
                                         lighting=False)
        actors.append(all_cfs_actor)

    if with_front:
        all_fronts_actor = plotter.add_mesh(combined_front, color='yellow', opacity=opacity, pickable=False,
                                            lighting=False)
...
def visualize_scenario(scenario: dict, log_dir: Path, output_dir: Path, with_intro=True,
    ...
    for i in range(0, len(timestamps), stride):
                ...
                for track_uuid, related_uuid_list in related_dict[timestamp].items():
                    ...
                    if relationship_edges:
                        relationship_edge_mesh = append_relationship_edges(relationship_edge_mesh,
                                                                           track_uuid, related_uuid_list, log_dir,
                                                                           timestamp, ego_to_city)
        ...
        # Add new cuboids
        scenario_actors = plot_cuboids(scenario_cuboids, plotter, ego_to_city, with_label=True, color='lime', opacity=1,
                                       with_cf=with_cf)
        related_actors = plot_cuboids(related_cuboids, plotter, ego_to_city, with_label=True, color='blue', opacity=1,
                                      with_cf=with_cf)
        other_actors = plot_cuboids(other_cuboids, plotter, ego_to_city, color='red', opacity=1, with_cf=with_cf)
        ego_actor = plot_cuboids(ego_cuboid, plotter, ego_to_city, color='red', with_label=True, opacity=1,
                                 with_cf=with_cf)
...
def set_camera_position_pv(plotter: pv.Plotter, scenario_dict: dict, relationship_dict: dict, log_dir):
    ...
    height_above_scenario = 1.1 * (np.linalg.norm(tr_corner - bl_corner)) / (
...
def append_relationship_edges(relationship_edge_mesh: pv.PolyData, track_uuid, related_uuids, log_dir, timestamp,
                              transform: SE3):
    df = read_feather(log_dir / 'sm_annotations.feather')
...
def animate_legend_intro(plotter: pv.plotter):
    plotter.add_legend([(' referred objects', 'lime', 'rectangle'), (' related objects', 'blue', 'rectangle'),
                        (' other objects', 'red', 'rectangle'), ('3', 'red')],
    ...
    plotter.add_legend([(' referred objects', 'lime', 'rectangle'), (' related objects', 'blue', 'rectangle'),
                        (' other objects', 'red', 'rectangle'), (' 2', 'orange')],
                       bcolor='white', border=True, loc='upper left', size=(.267, .13), font_family='times')
    ...
    plotter.add_legend([(' referred objects', 'lime', 'rectangle'), (' related objects', 'blue', 'rectangle'),
                        (' other objects', 'red', 'rectangle'), ('  1', 'yellow')],
                       bcolor='white', border=True, loc='upper left', size=(.267, .13), font_family='times')

    for j in range(3):
        plotter.write_frame()
    plotter.add_legend([(' referred objects', 'lime', 'rectangle'), (' related objects', 'blue', 'rectangle'),
                        (' other objects', 'red', 'rectangle'), ('   GO!', 'green')],
                       bcolor='white', border=True, loc='upper left', size=(.267, .13), font_family='times')
...
def plot_visualization_intro(plotter: pv.Plotter, scenario_dict: dict, log_dir, related_dict: dict[str, dict] = {},
                             description='scenario visualization'):
        ...
        for related_uuid, timestamps in related_objects.items():
            if timestamps and related_uuid in related_first_appearances:
                related_first_appearances[related_uuid] = min(min(timestamps), related_first_appearances[related_uuid])
                    ...
                    related_uuid not in track_first_appearences or min(timestamps) != track_first_appearences[
                related_uuid]):
